...
import { autoAddSimpleItems, autoAddLogisticsLegs, windowUpgradeGroup, mdyFromYmd } from "@/lib/intake/gsItems";
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  enqueueGsOp({ op: "create_project", label: `guided intake ${suggestEventName(intake)}`, payload: { intakeId: intake.id, eventName: suggestEventName(intake), notes: formatIntakeNotes(intake), details: gsEventDetails(intake), eventDateMDY: mdyFromYmd(intake.eventDate), addItems: autoAddSimpleItems(), windowUpgrade: windowUpgradeGroup(intake), location, logisticsLegs } });
