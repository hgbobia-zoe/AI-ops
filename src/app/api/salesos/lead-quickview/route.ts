// Compact lead detail for the board's quick-view slide-over (Jira-style peek without leaving the
// board). Read-only; money fields omitted for Members. Proxy-gated to signed-in console users.
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  const eventWhen =
    dte == null ? "No event date" : dte < 0 ? `${Math.abs(dte)} days ago` : dte === 0 ? "Today" : dte === 1 ? "Tomorrow" : `In ${dte} days`;
  ...
  return NextResponse.json({
    ...
    amountPaid: showMoney ? lead.amountPaid : null,
    amountDue: showMoney ? lead.amountDue : null,
