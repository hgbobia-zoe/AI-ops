...
function onWayText(stop: Stop, trackingUrl?: string): string {
  const name = stop.custName || "there";
  const track = trackingUrl ? ` Track your delivery here: ${trackingUrl}.` : "";
  const eta = stop.eta ? ` Estimated arrival: ${stop.eta}.` : "";
  return `Hi ${name} — your Zoe Events delivery team is on the way.${track}${eta} Reply to this message with any questions.`;
}

function arrivedText(stop: Stop): string {
  return `Hi ${stop.custName || "there"} — your Zoe Events delivery team has arrived. We'll begin unloading shortly.`;
}

async function sms(stop: Stop, body: string): Promise<void> {
  const r = await sendSms(stop.custPhone, body);
  insertMessage({
    stopId: stop.stopId,
    channel: "SMS",
    provider: "openphone",
    toPhone: stop.custPhone,
    body,
  ...
  else if (!r.ok) console.error("[fanout] SMS failed:", r.error);
}
  ...
  try {
    switch (ctx.action) {
      case "LEAVING_WAREHOUSE": {
        if (!cur) break;
        const t = createTracking(cur.stopId, cur.routeId, ctx.baseUrl);
        await sms(cur, onWayText(cur, t.url));
        await slack(`🚚 ${truck} departed → ${cur.custName}`);
        break;
      }
      case "ARRIVED": {
        if (!cur) break;
      ...
      case "HEADING_NEXT": {
        ...
        if (ctx.nextStop) {
          const t = createTracking(ctx.nextStop.stopId, ctx.nextStop.routeId, ctx.baseUrl);
          await sms(ctx.nextStop, onWayText(ctx.nextStop, t.url));
          await slack(`🚚 ${truck} heading to ${ctx.nextStop.custName}`);
        }
        break;
      }
