...
const BRIEF: Record<CustomerState, CallBriefTemplate> = {
  NEW: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Just sent your quote over, happy to walk through anything or tweak it to fit your event. What questions can I answer?",
  ...
  CONTACTED: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Following up on your quote, did it cover everything you had in mind, or is anything missing?",
  ...
  QUOTED: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events, checking in on your quote. Where's your thinking? Happy to answer anything that would help you decide.",
  ...
  EVALUATING: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. I know you're weighing your options, what are the main things on your list? Happy to help you compare apples to apples.",
  ...
  PRICE_OBJECTION: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Thanks for being honest about budget. Let's make the quote fit, want me to show a couple of options at different price points?",
  ...
  LOGISTICS_OBJECTION: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Let's lock the logistics, what are your delivery and pickup windows at the venue? I'll confirm we can hit them.",
  ...
  PRODUCT_UNCERTAINTY: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Want to make sure we spec the right setup, roughly how many guests, and indoor or outdoor? I'll tailor the quote to fit.",
  ...
  COMPETITOR_COMPARISON: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Totally fair to compare vendors. What are you weighing most? I'll be straight with you on where we're strong and what to watch for.",
  ...
  READY_TO_BOOK: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. Ready to lock you in, want me to send the contract now so we hold your date and everything on the quote?",
  ...
  DORMANT: {
    ...
    textDraft: "Hi {first}, this is {rep} from Zoe Events. No pressure at all, is your event still on? Happy to pick back up whenever the timing's right.",
...
export function callBriefFor(state: CustomerState, name: string, repFirst?: string | null): CallBriefTemplate {
  const first = (name || "").trim().split(/\s+/)[0] || "there";
  const rep = (repFirst ?? "").trim();
  const t = BRIEF[state];
  // Personalize: "Hi {first}, this is {rep} from Zoe Events." When the rep is known we use their first
  // name; when it isn't (login off / no name), we fall back to "it's Zoe Events" so it still reads right.
  const withRep = (s: string): string =>
    rep ? s.replace(/\{rep\}/g, rep) : s.replace(/this is \{rep\} from Zoe Events/g, "it's Zoe Events").replace(/\{rep\}/g, "Zoe Events");
  // Fill in the names, then run the house communication rule over every field: no dashes, no emoji,
  // natural. (humanize preserves the email body's line breaks.)
  const sub = (s: string): string => humanize(withRep(s.replace(/\{first\}/g, first)));
  return {
    ...t,
